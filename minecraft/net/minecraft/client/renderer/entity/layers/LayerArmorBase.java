...
import axyl.client.modules.features.visuals.Glint;
    ...
    private void renderGlint(EntityLivingBase entitylivingbaseIn, T modelbaseIn, float p_177183_3_, float p_177183_4_, float partialTicks, float p_177183_6_, float p_177183_7_, float p_177183_8_, float scale)
    {
        if (!Config.isShaders() || !Shaders.isShadowPass)
        {
            ...
            for (int i = 0; i < 2; ++i)
            {
                ...
                float f2 = 0.76F;
                Color clr = new Color(Glint.glintColor());
                if(Axyl.ins.modManager.getModuleByName("Glint").isToggled()) {
                 	GlStateManager.color(clr.getRed(), clr.getGreen(), clr.getBlue(), clr.getAlpha());
                } else {
                	GlStateManager.color(0.35F * f2, 0.25F * f2, 0.8F * f2, 1.0F);
                }
