...
import axyl.client.events.player.EventRayTrace;
...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        if (entity != null && this.mc.theWorld != null)
        {
            ...
            EventRayTrace eventRayTrace = new EventRayTrace(entity.rotationYaw, entity.rotationPitch);
            eventRayTrace.hook(eventRayTrace);
            
            this.mc.objectMouseOver = entity.rayTraceCustom((float) d0, partialTicks, eventRayTrace.getYaw(), eventRayTrace.getPitch());
            eventRayTrace.object = entity.rayTraceCustom((float) d0, partialTicks, eventRayTrace.getYaw(), eventRayTrace.getPitch());
            eventRayTrace.hook(eventRayTrace);
            ...
            Vec3 vec31 = entity.getVectorForRotation(eventRayTrace.pitch, eventRayTrace.yaw);
            ...
            eventRayTrace.entity = mc.pointedEntity;
            eventRayTrace.hook(eventRayTrace);
    ...
    private void hurtCameraEffect(float partialTicks)
    {
        ...
        {
        	if(Axyl.ins.modManager.getModuleByName("HurtCam").isToggled()) {
        		return;
        	}
            ...
            else
            {
                ...
                for (int i = 0; i < 8; ++i)
                {
                    ...
                    if (movingobjectposition != null)
                    {
                        ...
                        if (d7 < d3 && (!Axyl.ins.modManager.getModuleByName("ViewClip").isToggled())) {
                            d3 = d7;
                        }
                    }
                }
    ...
    public void updateCameraAndRender(float partialTicks, long nanoTime)
    {
        ...
        EventUpdateRotation eventUpdateRotation = new EventUpdateRotation();
        if(mc.thePlayer != null && mc.thePlayer.ticksExisted > 0) {
        	eventUpdateRotation.hook(eventUpdateRotation);
        }
        if (this.mc.inGameHasFocus && flag)
        {
            ...
            float f = this.mc.gameSettings.mouseSensitivity * 0.6F + 0.2F;
            float f1 = f * f * f * 8.0F;
            EventMouseDelta eventMouseDelta = new EventMouseDelta(this.mc.mouseHelper.deltaX, this.mc.mouseHelper.deltaY);
            eventMouseDelta.hook(eventMouseDelta);
            //fun
            /*if(eventMouseDelta.getDeltaX() > 3 || eventMouseDelta.getDeltaX() < -3) {
            	if(eventMouseDelta.getDeltaX() > 0) {
            		eventMouseDelta.setDeltaX(6);
            	} else {
            		eventMouseDelta.setDeltaX(-6);
            	}
            } else {
            	eventMouseDelta.setDeltaX(0);
            }
            if(eventMouseDelta.getDeltaY() > 3 || eventMouseDelta.getDeltaY() < -3) {
            	if(eventMouseDelta.getDeltaY() > 0) {
            		eventMouseDelta.setDeltaY(6);
            	} else {
            		eventMouseDelta.setDeltaY(-6);
            	}
            } else {
            	eventMouseDelta.setDeltaY(0);
            }*/
            float f2 = (float)eventMouseDelta.getDeltaX() * f1;
            float f3 = (float)eventMouseDelta.getDeltaY() * f1;
        ...
        if (!this.mc.skipRenderWorld)
        {
            ...
            if (this.mc.theWorld != null)
            {
                ...
                if (!this.mc.gameSettings.hideGUI || this.mc.currentScreen != null)
                {
                    ...
                    EventRender2D eventRender2D = new EventRender2D(scaledresolution);
                    eventRender2D.hook(eventRender2D);
    ...
    private void renderWorldPass(int pass, float partialTicks, long finishTimeNano)
    {
        ...
        eventRender3D.hook(eventRender3D);
    ...
    private void updateFogColor(float partialTicks)
    {
        ...
        Color c = new Color(Interface.worldFogColor.getValColor());
        this.fogColorRed = c.getRed()/255f;
        this.fogColorGreen = c.getGreen()/255f;
        this.fogColorBlue = c.getBlue()/255f;
