...
import axyl.client.modules.features.visuals.ItemSize;
...
public class ItemRenderer
{
    ...
    public void renderItemInFirstPerson(float partialTicks)
    {
        if (!Config.isShaders() || !Shaders.isSkipRenderHand())
        {
            ...
            double y = 0;
            double z = 0;
            if(Axyl.ins.modManager.getModuleByName("ItemSize").isToggled()) {
	            x = ItemSize.ItemX.getValDouble();
	            y = ItemSize.ItemY.getValDouble();
	            z = ItemSize.ItemZ.getValDouble();
            }
            GlStateManager.translate(x, y, z);
			final boolean block = KillAura.target != null && mc.thePlayer.getHeldItem() != null && mc.thePlayer.getHeldItem().getItem() != null && mc.thePlayer.getHeldItem().getItem() instanceof ItemSword;
            if (this.itemToRender != null)
            {
                ...
                else if (abstractclientplayer.getItemInUseCount() > 0 || (Axyl.ins.modManager.getModuleByName("KillAura").isToggled() && block))
                {
                    ...
                    switch (enumaction)
                    {
                        ...
                        case BLOCK:
                            this.transformFirstPersonItem(-0.06F, f1);
                            GlStateManager.translate(-0.5F, 0.2F, 0.0F);
                            GlStateManager.rotate(30.0F, 0.0F, 1.0F, 0.0F);
                            GlStateManager.rotate(-80.0F, 1.0F, 0.0F, 0.0F);
                            GlStateManager.rotate(60.0F, 0.0F, 1.0F, 0.0F);
