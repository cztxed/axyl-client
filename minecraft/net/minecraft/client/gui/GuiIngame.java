...
public class GuiIngame extends Gui
{
    ...
    public void renderGameOverlay(float partialTicks)
    {
        ...
        eventCrosshair.hook(eventCrosshair);
    ...
    private void renderScoreboard(ScoreObjective objective, ScaledResolution scaledRes)
    {
        ...
        int j1 = scaledRes.getScaledHeight() / 2 + i1 / 3+size;
        int k1 = 3;
        ...
        for (Score score1 : collection)
        {
            ...
            int k = j1 - j * (Fonts.opensans.getHeight()+2);
            int l = scaledRes.getScaledWidth() - k1 + 2;
            RenderUtil.drawRect(l1 - 2, k, l, this.getFontRenderer().FONT_HEIGHT, 1342177280);
            Fonts.opensans.drawString(s1, l1, k, 553648127);
            //this.getFontRenderer().drawString(s2, l - this.getFontRenderer().getStringWidth(s2), k, 553648127);

            if (j == collection.size())
            {
                String s3 = objective.getDisplayName();
                drawRect(l1 - 2, k - this.getFontRenderer().FONT_HEIGHT - 2, l, k, 0x7f101015);
                //drawRect(l1 - 2, k - 1, l, k, 1342177280);
                Fonts.opensans.drawString(s3, l1 + i / 2 - Fonts.opensans.getStringWidth(s3) / 2, k - this.getFontRenderer().FONT_HEIGHT, 553648127);
            }
        }
    }
