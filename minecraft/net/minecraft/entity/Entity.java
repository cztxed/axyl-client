...
{
	/* ac */
	public int pitch_changes = 0;
	public int yaw_changes = 0;
	public float lastYaw = 0;
	public float yaw = 0;
	public float lastPitch = 0;
	public float pitch = 0;
	public double lastS;
	public int eatingTicksPre;
	public int eatingTicksPost = 0;
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            if (this.isInWeb)
            {
                ...
                eventWeb.hook(eventWeb);
                if (Minecraft.getMinecraft().thePlayer == this) {
                    this.isInWeb = false;
                    if(!eventWeb.isCancelled()) {
                    	x *= eventWeb.getX();
	                    y *= eventWeb.getY();
	                    z *= eventWeb.getZ();
	                    this.motionX = 0.0D;
            ...
            eventSafeWalk.hook(eventSafeWalk);
    ...
    {
        EventStrafe eventStrafe = new EventStrafe(this.rotationYaw, strafe);
        eventStrafe.hook(eventStrafe);
        ...
        if (f >= 1.0E-4F)
        {
            ...
            if(this.equals(Minecraft.getMinecraft().thePlayer)) {
                ...
                f1 = MathHelper.sin((float) (eventStrafe.getYaw() * Math.PI / 180.0F));
            	f2 = MathHelper.cos((float) (eventStrafe.getYaw() * Math.PI / 180.0F));

            	this.motionX += (double)(strafe* f2 - forward * f1);
                this.motionZ += (double)(forward * f2 + strafe * f1);
            } else {
                f1 = MathHelper.sin((float) (this.rotationYaw * Math.PI / 180.0F));
            	f2 = MathHelper.cos((float) (this.rotationYaw * Math.PI / 180.0F));
	...
	public MovingObjectPosition rayTraceCustom(float blockReachDistance, float renderPartialTicks, float yaw, float pitch) {
        Vec3 vec3 = this.getPositionEyes(renderPartialTicks);
    ...
    public EnumFacing getHorizontalFacing(float dir)
    {
        return EnumFacing.getHorizontal(MathHelper.floor_double((double)(dir * 4.0F / 360.0F) + 0.5D) & 3);
    }
