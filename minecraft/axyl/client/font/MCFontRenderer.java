...
public class MCFontRenderer extends CFont {
    ...
    public int drawStringWithShadowNoFormmating(String text, double x2, double y2, int color) {

        double shadowWidth = this.drawStringNoFormmating(text, x2 +0.5f, (float) y2 + 0.5f, color, true, 8.3f);

        return (int) Math.max(shadowWidth, this.drawString(text, x2, y2, color, false, 8.3f));
    }
    
    public int drawStringNoFormmating(String text, double x2, double y2, int color) {
        return (int) this.drawStringNoFormmating(text, x2, y2, color, false, 8.3f);
    }
    
    public double drawStringNoFormmating(String text, double x, double y, int color, boolean shadow, double kerning) {
        x -= 1.0;
        ...
        GlStateManager.color((float) (color >> 16 & 255) / 255f, (float) (color >> 8 & 255) / 255f, (float) (color & 255) / 255f, (float) alpha);
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character < currentData.length) {
                ...
                if (strikethrough) {
                    this.drawLine(x, y + (float) (currentData[character].height / 2), x + (float) currentData[character].width - 8, y + (float) (currentData[character].height / 2), 1);
                }
    ...
    public int drawStringWithShadow(String text, double x2, double y2, int color) {
        ...
        double shadowWidth = this.drawString(text, x2 +0.5f, (float) y2 + 0.5f, color, true, 8.3f);

        return (int) Math.max(shadowWidth, this.drawString(text, x2, y2, color, false, 8.3f));
    }
    
    public static void drawStringWithSolidShadow(MCFontRenderer font, String text, double x, double y, int color) {
     	font.drawString(text, x - 0.5f, y, 0xff000000);
    	font.drawString(text, x + 0.5f, y, 0xff000001);
    	font.drawString(text, x, y - 0.5f, 0xff000000);
    	font.drawString(text, x, y + 0.5f, 0xff000001);
    	font.drawString(text, x, y, color);
    }
    public int drawString(String text, double x2, double y, int color) {
        return (int) this.drawString(text, x2, y, color, false, 8.3f);
    }

    public int drawPassword(String text, double x2, double y2, int color) {
        return (int) this.drawString(text.replaceAll(".", "."), x2, y2, color, false, 8f);
    }

    public int drawNoBSString(String text, double x2, double y2, int color) {
        return (int) this.drawNoBSString(text, x2, y2, color, false);
    }

    public int drawSmoothString(String text, double x2, double y2, int color) {
    ...
    public double drawCenteredString(String text, double x2, double y2, int color) {
        return this.drawString(text, x2 - (float) (this.getStringWidth(text) / 2), y2, color);
    }

    public double drawNoBSCenteredString(String text, double x2, double y2, int color) {
        return this.drawNoBSString(text, x2 - (float) (this.getStringWidth(text) / 2), y2, color);
    }

    public double drawCenteredStringWithShadow(String text, double x2, double y2, int color) {
        return this.drawStringWithShadow(text, x2 - (float) (this.getStringWidth(text) / 2), y2, color);
    }

    public double drawString(String text, double x, double y, int color, boolean shadow, double kerning) {
        x -= 1.0;
        ...
        for (int index = 0; index < text.length(); index++) {
            ...
            if (character == '\u00a7') {
                ...
                if (colorIndex < 16) {
                    ...
                    GlStateManager.color((float) (colorcode >> 16 & 255) / 255f, (float) (colorcode >> 8 & 255) / 255f, (float) (colorcode & 255) / 255f, (float) alpha);
    ...
    public double drawSmoothString(String text, double x, double y, int color, boolean shadow) {
        x -= 1;
    ...
    public double drawNoBSString(String text, double x, double y, int color, boolean shadow) {
        x -= 1;
    ...
    public double getStringWidth(String text, double kerning) {
        if (text == null) {
            return 0;
        }
    ...
    private void drawLine(double x2, double y2, double x1, double y1, double width) {
    ...
    private double getCharWidthFloat(char c) {
        if (c == 167) {
            return -1;
        } else if (c == 32) {
            return 2;
    ...
    public String trimStringToWidth(String text, int width, boolean custom) {
        ...
        for (int index = offset; index >= 0 && index < text.length() && lineWidth < (float) width; index += increment) {
            char character = text.charAt(index);
            double charWidth = this.getCharWidthFloat(character);
